...
export default function ClientStatusPortal() {
  ...
  const checkStatus = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('System Uplink Failure. Verify credentials.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="status-container">
      {/* THE FORTRESS: Inline CSS ensures this looks perfect even if Tailwind fails */}
      ...
      <div className="status-card">
        {!record ? (
          <form onSubmit={checkStatus}>
            ...
            <p style={{ color: '#94a3b8', fontSize: '13px', marginBottom: '35px' }}>Access your mandate progression via secure uplink.</p>
          ...
          <div>
            ...
            <button className="btn" style={{ marginTop: '30px', background: '#000' }} onClick={() => setRecord(null)}>
              TERMINATE SESSION
            </button>
          </div>
        )}
        <Link href="/" className="exit-link">RETURN TO LOBBY</Link>
      </div>
    </div>
  );
}